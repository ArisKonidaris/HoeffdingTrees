...
import ht.math.{LabeledPoint, UnlabeledPoint, Vector}
import ht.trees.nodes.{InternalNode, LeafNode, Node, TestAttribute}
import ht.trees.serializable.HTDescriptor
import ht.trees.serializable.nodes.{InternalNodeDescriptor, LeafNodeDescriptor}
import ht.trees.stats.{DiscreteStatistics, NumericalStatistics}
...
class HoeffdingTree(private var discrete: Boolean = false,
  ...
  def setNumberOfDataPointsSeen(dataPointsSeen: Long): Unit = this.dataPointsSeen = dataPointsSeen
  ...
  def updateLeaf(leaf: LeafNode,
                 ...
                 memoryCheck: Boolean = true): Unit = {
    if (leaf.predict(dataPoint)._1 != target) leaf.errors += 1 // Update the error counter of the filtered leaf.
   ...
   */
  def fit(point: LabeledPoint, splitting: Boolean = true, memoryCheck: Boolean = true): Unit = {
    ...
    if (canTrain) {
      ...
      updateLeaf(fNode, dataPoint, target, splitting, memoryCheck)
   ...
   */
  def fit(batch: ListBuffer[LabeledPoint]): Unit = for (point: LabeledPoint <- batch) fit(point)
  ...
  // TODO: Implement more sophisticated metrics than the accuracy for testing the performance of the HT.
  /**
   * Calculating the accuracy of the predicting performance of the Hoeffding Tree.
   *
   * @param test_set  The test set used to calculate the predictive performance of the Hoeffding Tree.
   * @param test_type The method for calculating the predictive performance of the Hoeffding Tree.
   * @return The predictive performance of the Hoeffding Tree.
   */
  def score(test_set: ListBuffer[LabeledPoint], test_type: String = "accuracy"): Double = {
    test_type match {
      case "accuracy" =>
        try {
          if (test_set.nonEmpty) {
            val v = {
              for (test <- test_set) yield {
                val prediction: Double = predict(test.convertToUnlabeledPoint())._1
                if (test.label == prediction) 1.0 else 0.0
              }
            }
            v.sum / test_set.length
          } else Double.NaN
        } catch {
          case _: Throwable => Double.NaN
        }
      case "f1-score" => Double.NaN
    }
  }
