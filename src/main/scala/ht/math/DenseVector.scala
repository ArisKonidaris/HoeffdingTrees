...
case class DenseVector(var data: Array[Double]) extends Vector with Serializable {
   ...
   * with `other`. The result will given request [[ht.math.SparseMatrix]]
   * representation if `other` is sparse and as [[ht.math.DenseMatrix]] otherwise.
   *
   * @param other a Vector
   * @return the [[ht.math.Matrix]] which equals the outer product of `this`
   *         with `other.`
   */
...
object DenseVector {
  ...
  /** BreezeVectorConverter implementation for [[ht.math.DenseVector]]
   *
